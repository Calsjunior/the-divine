...
result=$(awk -v min="$min_val" -v pref="$preferred_val" -v max="$max_val" -v smin="$smin" -v smax="$smax" 'BEGIN {
    if (smin > smax) {
        printf "clamp(%s, %s, %s)", max, pref, min
    } else {
        printf "clamp(%s, %s, %s)", min, pref, max
    }
}')
